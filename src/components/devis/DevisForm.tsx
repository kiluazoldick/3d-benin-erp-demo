...
import type { Devis, LigneDevis, StatutDevis } from "@/lib/types";
...
function generateNumero(): string {
  const all = readCollection<Devis>("devis");
  const year = new Date().getFullYear();
  const prefix = `DEV-${year}-`;
  const nums = all
    .filter((d) => d.numero.startsWith(prefix))
    .map((d) => parseInt(d.numero.replace(prefix, ""), 10) || 0);
...
export default function DevisForm({ initial }: { initial?: Devis }) {
  ...
  const [form, setForm] = useState<Omit<Devis, "id" | "createdAt">>({
    ...
    dateValidite: initial?.dateValidite?.split("T")[0] ?? "",
    lignes: initial?.lignes?.length ? initial.lignes : [emptyLigne()],
    totalHT: initial?.totalHT ?? 0,
    tva: initial?.tva ?? 0,
    totalTTC: initial?.totalTTC ?? 0,
  ...
  const updateLigne = (idx: number, patch: Partial<LigneDevis>) => {
    setForm((f) => ({
      ...f,
      lignes: f.lignes.map((l, i) => (i === idx ? { ...l, ...patch } : l)),
    }));
  };
  ...
  const removeLigne = (idx: number) => {
    setForm((f) => ({
      ...f,
      lignes: f.lignes.filter((_, i) => i !== idx),
    }));
  };
  ...
  const validate = () => {
    ...
    if (form.lignes.length === 0) e.lignes = "Ajoutez au moins une ligne";
    if (form.lignes.some((l) => !l.designation.trim()))
      e.lignes = "Toutes les lignes doivent avoir une désignation";
    if (form.lignes.some((l) => l.quantite <= 0 || l.prixUnitaire <= 0))
  ...
  const onSubmit = (ev: React.FormEvent) => {
    ...
    const payload = {
      ...
      dateValidite: form.dateValidite
        ? new Date(form.dateValidite).toISOString()
        : "",
      totalHT: totaux.ht,
      tva: totaux.tva,
      totalTTC: totaux.ttc,
    ...
    if (initial) {
      updateItem<Devis>("devis", initial.id, payload);
    } else {
      createItem<Devis>("devis", payload);
    }
    refresh();
    router.push("/devis");
  };

  // Chantiers du client sélectionné uniquement
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <Select
            label="Statut"
            value={form.statut}
            onChange={(e) => set("statut", e.target.value as StatutDevis)}
            options={STATUTS}
          />
          <Input
      ...
      >
        ...
        {/* En-tête desktop */}
        <div
          className="hidden md:grid grid-cols-12 gap-3 px-3 pb-3 text-[11px] font-semibold uppercase"
        ...
        <div className="flex flex-col gap-3">
          {form.lignes.map((l, idx) => {
            const ligneTotal = l.quantite * l.prixUnitaire;
            return (
              <div
                key={idx}
              ...
              >
                <div className="md:col-span-5">
                  <label
                    className="md:hidden block text-[11px] font-semibold mb-1"
                    style={{ color: C.slate }}
                  >
                    Désignation
                  </label>
                  <input
                    ...
                    placeholder="Ex. Terrassement voirie"
                ...
                <div className="md:col-span-2 text-right">
                  <label
                    className="md:hidden block text-[11px] font-semibold mb-1 text-left"
                    style={{ color: C.slate }}
                  >
                    Total
                  </label>
                ...
                <div className="md:col-span-1 flex md:justify-end">
                  <button
                    ...
                    className="grid place-items-center h-10 w-10 rounded-lg transition-opacity"
                    style={{
                      color: C.danger,
                      opacity: form.lignes.length === 1 ? 0.3 : 1,
                      cursor:
                        form.lignes.length === 1 ? "not-allowed" : "pointer",
                    }}
